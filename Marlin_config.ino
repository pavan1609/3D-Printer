...
// Limit switch pins
const int LIMIT_X_PIN = 47;
const int LIMIT_Y_PIN = 45;
const int LIMIT_Z_PIN = 32;
...
File myFile;
int line[100];
float d1, d2, d3, xMov, yMov, zMov, xMov1, yMov1, zMov1, xOut, yOut, zOut, xPast, yPast, zPast, DELAY;

void setup();
void loop();
void calibrate();
void motorX(int direction, float delay);
void motorY(int direction, float delay);
void motorZ(int direction, float delay);
void parseValues(int line[], int size);
float modify(int x[], int size);
void xyz2delta();
void xyDelta(float newX, float newY);
float distance(float a, float b, float c, float d);
void zDelta(float newZ);
float machineConversion(float xMove, float yMove, float zMove);
void motorRun(float x, float y, float z);
...
void loop() {
  // No implementation needed in the loop function
}
...
void motorX(int direction, float delay) {
  // Motor X control code...
}

void motorY(int direction, float delay) {
  // Motor Y control code...
}

void motorZ(int direction, float delay) {
  // Motor Z control code...
}
...
void xyDelta(float newX, float newY) {
  // XY delta calculation code...
}

float machineConversion(float xMove, float yMove, float zMove) {
  // Machine conversion code...
}
